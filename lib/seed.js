...
export async function ensureSeedAndMigration(db) {
  ...
  if (workspacesCount === 0) {
    ...
    await ensureDefaultChannels(db, workspaceId, groups)

    return { migrated: true, workspaceId, memberCount: members.length }
  }

  // Ensure channels exist for every workspace (idempotent)
  const allWs = await db.collection('workspaces').find({}).toArray()
  for (const ws of allWs) {
    const chCount = await db.collection('channels').countDocuments({ workspaceId: ws.id })
    if (chCount === 0) {
      const groups = await db.collection('groups').find({ workspaceId: ws.id }).toArray()
      await ensureDefaultChannels(db, ws.id, groups)
    }
  }
...
async function ensureDefaultChannels(db, workspaceId, groups) {
  const channels = [
    { name: 'general', type: 'workspace', description: 'Discussion générale' },
    { name: 'chefs', type: 'leaders', description: 'Échanges entre chefs et admins' },
  ]
  for (const g of groups) {
    channels.push({ name: (g.name || 'groupe').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''), type: 'group', description: `Channel du groupe ${g.name}`, groupId: g.id })
  }
  ...
  if (docs.length) await db.collection('channels').insertMany(docs)
}
