...
// Notification IDs include legacy donation IDs; never broaden task/resource IDs.
export function notificationId(value) {
  const uuid = '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}'
  if (typeof value === 'string' && new RegExp(`^donation:${uuid}:${uuid}$`, 'i').test(value)) return value
  return id(value, 'Identifiant de notification')
}
